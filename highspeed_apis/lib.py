...
def streaming_func(asdf):
    ...
    while(time.time() < (current_time + (60*60*9))):
        try:
            result = json.loads(zlib.decompress(ws.recv(), 16+zlib.MAX_WBITS).decode('utf8'))
        except:
            pass
...
def write_to_target(asdf):
    ...
    while (time.time() < (current_time + (60*60*9)+30)):
        time.sleep(15)
        ...
        with open(str('data/incremental_data_two/' + str(count) + '.pkl'), 'wb') as handle: pickle.dump(d, handle, protocol=pickle.HIGHEST_PROTOCOL)
...
with open('data/lastnight.pkl', 'wb') as handle: pickle.dump(written_to_file, handle, protocol=pickle.HIGHEST_PROTOCOL)
